...
def save_articles_to_json(articles_by_country, filename="most_viewed_articles_24h.json"):
    ...
    output_data = {
        'metadata': {
            ...
            'total_articles': sum(len(articles) for articles in articles_by_country.values())
        },
...
def calculate_average_sentiment_per_country(input_filename="most_viewed_articles_24h.json", 
    ...
    try:
        ...
        for country, articles in articles_by_country.items():
            ...
            if sentiments:
                ...
                print(f"📍 {country}: {avg_sentiment:.4f} (from {len(sentiments)} articles)")
            else:
        ...
        if average_sentiment:
            print(f"\n📈 SENTIMENT SUMMARY:")
            sorted_countries = sorted(average_sentiment.items(), key=lambda x: x[1], reverse=True)
            print(f"Most positive: {sorted_countries[0][0]} ({sorted_countries[0][1]:.4f})")
            print(f"Most negative: {sorted_countries[-1][0]} ({sorted_countries[-1][1]:.4f})")
...
def get_single_country_articles(country_name, max_articles=20):
    ...
    if articles:
        ...
        for i, article in enumerate(articles[:5], 1):
            ...
            print(f"   Social Score: {article['socialScore']} | Source: {article['source']}")
...
if __name__ == "__main__":
    ...
    results = get_most_viewed_articles_multiple_countries(major_countries[:5], articles_per_country=5)
